...
| `PHCOMP` | Philippines (Luzon, Visayas, Mindanao) | PAGASA PANAHON (8-radar mosaic) | 0.0069° lon / 0.0091° lat (~770m) | ~4 MB |

Group aliases: `CONUS` (continental US only), `US` (all US regions), `CANADA` (Canada), `CENTRAL_AMERICA` (El Salvador + W. Honduras + S. Guatemala + offshore Pacific), `EUROPE` (OPERA pan-European composite), `SOUTHEAST_ASIA` (MET Malaysia peninsular + east composites covering Peninsular Malaysia, Borneo, Brunei, N. Sumatra + PAGASA PANAHON mosaic covering the Philippines), `TAIWAN` (CWA QPESUMS composite covering Taiwan + offshore Pacific), `ALL` (everything).
...
- **[PAGASA](https://www.pagasa.dost.gov.ph/)** — Philippine Atmospheric, Geophysical and Astronomical Services Administration, 8-radar national mosaic via the PANAHON web app's anonymous CloudFront-fronted API at `cdn.panahon.gov.ph`. Public domain per Philippine IP code RA 8293 §176 (government-works exception); attributed to PAGASA / DOST.
...
- **Limited radar coverage outside US / Canada / Europe / Central America / Taiwan / SE Asia** — real radar composites cover the US (CONUS, Alaska, Hawaii, Puerto Rico, Guam), Canada, El Salvador and its neighbours, Europe (via OPERA), Taiwan (CWA QPESUMS), Malaysia + Borneo + Brunei + Singapore + N. Sumatra (MET Malaysia), and the Philippines (PAGASA PANAHON). Everywhere else uses the regional NWP chain on top of ECMWF IFS for the precipitation layer — that's a complete picture of global precipitation, but it's modelled output, not direct radar observation
