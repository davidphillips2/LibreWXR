...
src/librewxr/
  ...
  data/
    ...
    pagasa_source.py # PAGASA Philippines radar source (split out of sources.py)
...
- **Multi-region:** US (USCOMP, AKCOMP, HICOMP, PRCOMP, GUCOMP), Canada (CACOMP), Central America (SVCOMP), Europe (OPERA), Taiwan (TWCOMP), SE Asia (MYPENINSULAR, MYEAST, PHCOMP)
...
- **PAGASA (Philippines):** 8-radar PANAHON national mosaic covering Luzon + Visayas + Mindanao + W. Palawan via anonymous CloudFront-fronted JSON+PNG API at `cdn.panahon.gov.ph`; 15-min native cadence (the 10-min store grid rounds to the nearest native frame — ≤7.5 min off, invisible in animation); single timeline endpoint returns 6 frames with explicit UTC `observed_at_unix` timestamps so no publish-lag math; 2048×2048 RGBA PNGs in EPSG:4326; 13-stop linear 0–75 dBZ palette extracted from the JS bundle's `generateGradientColormap` call; first three palette stops are weak-echo grays with α<255 in the source PNG and decode to no-data; CSRF token in URL is decorative (the CDN endpoint ignores it); single PHCOMP region in SOUTHEAST_ASIA group, peer to MMD; public domain per Philippine IP code RA 8293 §176; lives in `data/pagasa_source.py`
